<template>
    <div class="drawer-layout">
      <div class="drawer-wrap" :class="animateStyleDrawer" :style_="drawerStyle">
        <slot name="drawer" />
      </div>
      <div
        class="content-wrap"
        :class="contentDrawable ? animateStyle : {}"
  ...
  export default {
    ...
    methods: {
      toggle(visible) {
        ...
        this.pos = visible ? this.width : 30;
        this.moving = true;
      },
      handleMaskClick() {
    ...
    computed: {
      ...
      drawerStyle() {
        ...
        return {
          ...
          transform: `translate3d(${reverse ? "-" : ""}${Math.ceil(
            pos * moveRate
          )}px,0,0)`
        };
      },
    ...
    mounted() {
      ...
      const initDrag = function(e) {
        ...
        document.addEventListener(
          mouseEvents.move,
          drag,
          isTouch && supportsPassive ? { passive: true } : false
        );
        document.addEventListener(
          mouseEvents.up,
          removeDrag,
      ...
      const drag = function(e) {
        ...
        if (isVerticle === undefined)
          isVerticle =
            Math.abs(nowX - startX) / Math.abs(nowY - startY) < Math.sqrt(3);
        if (!isVerticle) {
      ...
      const removeDrag = function(e) {
        if (isVerticle !== undefined) {
          if (!isVerticle) {
            ...
            if (speed > 0) {
              this.visible =
                (width - pos) / speed < duration || pos > (width * 3) / 5;
            } else {
              this.visible = !(
                (0 - pos) / speed < duration || pos < (width * 3) / 5
              );
            }
            if (this.pos > 0 && this.pos < width) this.moving = true;
          }
          this.pos = this.visible ? width : 30;
        }
        if (!this.moving) {
